...
select *from sailors;
create table boat(BID numeric unique,sname varchar(20),color varchar(20));
...
select *from boat;
create table reserve(RID numeric unique,SID numeric,BID numeric,rdate date,duration int);
...
select*from reserve;
select BID,count(*) from reserve group by BID;
select age from sailors where age>= all (select age from sailors);
select BID,count(boat) from boat;
select age,avg(distinct age) as avgage from sailors;
select rating,sum(rating) from sailors;
select age,avg(age) from sailors;
select age,sum(distinct age) from sailors;
select age from sailors where age<= all (select age from sailors);
select*from sailors natural join reserve;
select*from sailors inner join reserve using(SID);
select*from sailors left outer join reserve using(SID);
select*from sailors right outer join reserve using(SID);